...
class ERGNN(Replay):
    ...
    def memorize(self, task, budgets):
        ...
        sim = [centers[i].view(1, -1).mm(emb[ids_per_cls_train[i]].permute(1, 0)).squeeze() for i in
               range(len(centers))]
        ...
        replayed_graph = Data(x=task.x[ids_selected], y=task.y[ids_selected],
                              adj_t=SparseTensor.eye(replayed_graph_size, replayed_graph_size).t())
